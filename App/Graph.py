...
def Update():
    try:
        ...
        if LastContent != Content:
            ...
            YAxisScale = max(5, round(Variables.GraphZoom * 5))

            for i in range(XAxisScale + 1):
                FloatX = i / XAxisScale
                X, Y = ConvertToImageCoordinate(FloatX, 1)
                if -10 <= X <= Variables.Graph.shape[1] + 9:
                    ...
                    Text, Fontscale, Thickness, Width, Height = GetTextSize(f"{int(MinX + (XAxisMax - MinX) * FloatX):,}".replace(",", "."), 1000, 10)
                    Y = round(Y + Height * 1.5)
                    if Y >= Variables.Graph.shape[0] - 2:
                        Y = Variables.Graph.shape[0] - 2
                    cv2.putText(Frame,
                                Text,
                                (round(X - Width / 2), Y),
            ...
            for i in range(YAxisScale + 1):
                ...
                if -10 <= Y <= Variables.Graph.shape[0] + 9:
                    ...
                    Text, Fontscale, Thickness, Width, Height = GetTextSize(f"{round(MaxY - (MaxY - MinY) * FloatY, max(2, min(9, round(3 * (Variables.GraphZoom / 100) * 100)))):,}".replace(",", "#").replace(".", ",").replace("#", "."), 1000, 10)
                    X = round(X - Width * 1.1)
                    if X < 1:
                        X = 1
                    cv2.putText(Frame,
